...
class Item2ViewController: UIViewController {
    ...
    @IBOutlet weak var lowerACTlabel: UILabel!
    @IBOutlet weak var averageACTlabel: UILabel!
    @IBOutlet weak var upperACTlabel: UILabel!
    @IBOutlet weak var averageSATlabel: UILabel!
    @IBOutlet weak var admissionLabel: UILabel!
    @IBOutlet weak var retentionRateLabel: UILabel!
    @IBOutlet weak var backBtn: UIButton!
    @IBOutlet weak var logoView: UIImageView!
    
    var admissionRate :Double = 0.0
    var admissionRateFormat :Double = 0.0
    
    var retRate :Double = 0.0
    var retRateFormat :Double = 0.0
    ...
    override func viewDidLoad()
    {
       ...
       schoolName.minimumScaleFactor = 0.5
        ...
        lowerACTlabel.layer.masksToBounds = true
        lowerACTlabel.layer.cornerRadius = 6
        averageACTlabel.layer.masksToBounds = true
        averageACTlabel.layer.cornerRadius = 6
        upperACTlabel.layer.masksToBounds = true
        upperACTlabel.layer.cornerRadius = 6
        averageSATlabel.layer.masksToBounds = true
        averageSATlabel.layer.cornerRadius = 6
        admissionLabel.layer.masksToBounds = true
        admissionLabel.layer.cornerRadius = 6
        retentionRateLabel.layer.masksToBounds = true
        retentionRateLabel.layer.cornerRadius = 6
        ...
        URLSession.shared.dataTask(with: jsonUrlString) { (data, response, err) in
            ...
            DispatchQueue.main.async {
                ...
                if finalLowerACT == 0 {
                    self.lowerACTlabel.text = "Data Not Available"
                    self.lowerACTlabel.font = UIFont(name: "kefa", size: 14.0)
                } else {
                    self.lowerACTlabel.text = "\(finalLowerACT)"
                }
                
                if finalACT == 0 {
                    self.averageACTlabel.text = "Data Not Available"
                    self.averageACTlabel.font = UIFont(name: "kefa", size: 14.0)
                } else {
                    self.averageACTlabel.text = "\(finalACT)"
                }
                
                if finalUpperACT == 0 {
                    self.upperACTlabel.text = "Data Not Available"
                    self.upperACTlabel.font = UIFont(name: "kefa", size: 14.0)
                } else {
                    self.upperACTlabel.text = "\(finalUpperACT)"
                }
                
                if finalSAT == 0 {
                    self.averageSATlabel.text = "Data Not Available"
                    self.averageSATlabel.font = UIFont(name: "kefa", size: 14.0)
                } else {
                    self.averageSATlabel.text = "\(finalSAT)"
                }
                
               
                
                if finalADMISSION == 0.0 {
                    self.admissionLabel.text = "Data Not Available"
                    self.admissionLabel.font = UIFont(name: "kefa", size: 14.0)
                } else {
                    self.admissionRate = finalADMISSION * 100
                    self.admissionRateFormat = Double(round(100 * self.admissionRate) / 100)
                    self.admissionLabel.text = "\(self.admissionRateFormat)%"
                }
                
                if finalretRate == 0.0 {
                    self.retentionRateLabel.text = "Data Not Available"
                    self.retentionRateLabel.font = UIFont(name: "kefa", size: 14.0)
                } else {
                    self.retRate = finalretRate * 100
                    self.retRateFormat = Double(round(100 * self.retRate) / 100)
                    self.retentionRateLabel.text = "\(self.retRateFormat)%"
                }
    ...
    @IBAction func lowerACTInfo(_ sender: UIButton)
    {
        let lowerACTAlert = UIAlertController(title: "25th Percentile ACT", message: "This is the score that is higher than what 25% of admitted students scored", preferredStyle: .alert)
        let ok = UIAlertAction(title: "Done", style: .cancel)
        lowerACTAlert.addAction(ok)
        present(lowerACTAlert, animated: true, completion: nil)
    }
    
    @IBAction func midpointACTInfo(_ sender: UIButton)
    {
    ...
    @IBAction func upperACTInfo(_ sender: UIButton) {
        let upperACTAlert = UIAlertController(title: "75th Percentile ACT", message: "This is the score that is higher than what 75% of the admitted students scored", preferredStyle: .alert)
        let ok = UIAlertAction(title: "Done", style: .cancel)
        upperACTAlert.addAction(ok)
        present(upperACTAlert, animated: true, completion: nil)
    }
    
    
    @IBAction func midpointSATInfo(_ sender: UIButton)
    {
