//
//  CompareScreenViewController.swift
//  College App (page based)
//
//  Created by Eric Markey on 6/24/19.
...
var school1GradRate = 0.0
var school1Salary = 0
var school1Cost = 0
var school1ACTINT = 0
var school1SAT = 0
var school1AdmRate = 0.0
var school1RETRATE = 0.0
var school1Enroll = 0

var school2GradRate = 0.0
var school2Salary = 0
var school2Cost = 0
var school2ACTINT = 0
var school2SAT = 0
var school2AdmRate = 0.0
var school2RetentionRate = 0.0
var school2Enroll = 0


class CompareScreenViewController: UIViewController {
    @IBOutlet weak var school1GradRateLabel: UILabel!
    @IBOutlet weak var school1SalaryLabel: UILabel!
    @IBOutlet weak var school1CostLabel: UILabel!
    @IBOutlet weak var homeBTN: UIButton!
    @IBOutlet weak var school2CostLabel: UILabel!
    @IBOutlet weak var school2GradRateLabel: UILabel!
    @IBOutlet weak var school2SalaryLabel: UILabel!
    @IBOutlet weak var school1EnrollLabel: UILabel!
    @IBOutlet weak var school2EnrollLabel: UILabel!
    
    
    var completion :Double = 0.0
    var completionRate :Double = 0.0
    
    var completion2 :Double = 0.0
    var completionRate2 :Double = 0.0
...
//        newSchoolBTN.layer.cornerRadius = 8
        ...
        school1Name1.minimumScaleFactor = 0.5
        school1Name2.adjustsFontSizeToFitWidth = true
        school1Name2.minimumScaleFactor = 0.5
        school1Name3.adjustsFontSizeToFitWidth = true
        school1Name3.minimumScaleFactor = 0.5
        school1Name4.adjustsFontSizeToFitWidth = true
        school1Name4.minimumScaleFactor = 0.5
        ...
        school1CostLabel.layer.masksToBounds = true
        school1CostLabel.layer.cornerRadius = 6
        school1SalaryLabel.layer.masksToBounds = true
        school1SalaryLabel.layer.cornerRadius = 6
        school1GradRateLabel.layer.masksToBounds = true
        school1GradRateLabel.layer.cornerRadius = 6
        
        school2CostLabel.layer.masksToBounds = true
        school2GradRateLabel.layer.masksToBounds = true
        school2SalaryLabel.layer.masksToBounds = true
        school2CostLabel.layer.cornerRadius = 6
        school2GradRateLabel.layer.cornerRadius = 6
        school2SalaryLabel.layer.cornerRadius = 6
        school1EnrollLabel.layer.masksToBounds = true
        school1EnrollLabel.layer.cornerRadius = 6
        school2EnrollLabel.layer.masksToBounds = true
        school2EnrollLabel.layer.cornerRadius = 6
      ...
      ///////////////
        let compare1URL = urlSchool1
        URLSession.shared.dataTask(with: compare1URL!) { (data, response, err) in
            ...
            do {
                ...
                let results = compareResults(json: json1)
            ...
            DispatchQueue.main.async
                {
                    ...
                    if school1Cost == 0 {
                        self.school1CostLabel.text = "Data Not Available"
                        self.school1CostLabel.font = UIFont(name: "kefa", size: 14.0)
                    } else {
                        let result = NumberFormatter.localizedString(from: NSNumber(value: school1Cost), number:.currency )
                        self.school1CostLabel.text = "\(result)"
                    }

                    if school1GradRate == 0.0 {
                        self.school1GradRateLabel.text = "Data Not Available"
                        self.school1GradRateLabel.font = UIFont(name: "kefa", size: 14.0)
                    } else {
                        self.completion = school1GradRate * 100
                        self.completionRate = Double(round(100 * self.completion) / 100)
                        self.school1GradRateLabel.text = "\(self.completionRate)%"
                    }

                    if school1Salary == 0 {
                        self.school1SalaryLabel.text = "Data Not Available"
                        self.school1SalaryLabel.font = UIFont(name: "kefa", size: 14.0)
                    } else {
                         let earn = NumberFormatter.localizedString(from: NSNumber(value: school1Salary), number: .currency)
                        self.school1SalaryLabel.text = "\(earn)"
                    }
                    
                    if school1Enroll == 0 {
                        self.school1EnrollLabel.text = "Data Not Available"
                        self.school1EnrollLabel.font = UIFont(name: "kefa", size: 14.0)
                    } else {
                        let numberformatter = NumberFormatter()
                        numberformatter.numberStyle = .decimal
                        let formatEnroll1 = numberformatter.string(from: NSNumber(value: school1Enroll))
                        self.school1EnrollLabel.text = formatEnroll1
        ...
        let compare2URL = urlSchool2
        URLSession.shared.dataTask(with: compare2URL!) { (data, response, err) in
            ...
            do {
                ...
                let results = secondCompareResults(json: json1)
            ...
            DispatchQueue.main.async
                {
                    ...
                    if school2Cost == 0 {
                        self.school2CostLabel.text = "Data Not Available"
                        self.school2CostLabel.font = UIFont(name: "kefa", size: 14.0)
                    } else {
                        let result = NumberFormatter.localizedString(from: NSNumber(value: school2Cost), number:.currency )
                        self.school2CostLabel.text = "\(result)"
                    }
                    
                    
                    if school2Salary == 0 {
                        self.school2SalaryLabel.text = "Data Not Available"
                        self.school2SalaryLabel.font = UIFont(name: "kefa", size: 14.0)
                    } else {
                        let earn = NumberFormatter.localizedString(from: NSNumber(value: school2Salary), number: .currency)
                        self.school2SalaryLabel.text = "\(earn)"
                    }

                    if school2GradRate == 0.0 {
                        self.school2GradRateLabel.text = "Data Not Available"
                        self.school2GradRateLabel.font = UIFont(name: "kefa", size: 14.0)
                    } else {
                        self.completion2 = school2GradRate * 100
                        self.completionRate2 = Double(round(100 * self.completion2) / 100)
                        self.school2GradRateLabel.text = "\(self.completionRate2)%"
                    }
                    
                    if school2Enroll == 0 {
                        self.school2EnrollLabel.text = "Data Not Available"
                        self.school2EnrollLabel.font = UIFont(name: "kefa", size: 14.0)
                    } else {
                        let numberformatter = NumberFormatter()
                        numberformatter.numberStyle = .decimal
                        let formatEnroll2 = numberformatter.string(from: NSNumber(value: school2Enroll))
                        self.school2EnrollLabel.text = formatEnroll2
                    }
    ...
    {
        performSegue(withIdentifier: "selectNewSchools", sender: nil)
    }
    ...
    {
        print("Back to home")
        performSegue(withIdentifier: "homeFromCompare", sender: nil)
    }
